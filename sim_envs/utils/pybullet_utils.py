...
class UrdfObject:
    """
    用于包装一个物理对象, 自动处理惯性坐标偏移的问题 (没考虑旋转和多连杆的情况)
    """
    def __init__(self, obj_id, scaling=1.0):
        """
        初始化一个物理对象
        Args:
            obj_id (int): 物体在PyBullet中的唯一ID。
            scaling (float): 物体加载时使用的全局缩放。
        """
        self.obj_id = obj_id

        # --- 获取惯性偏移 ---
        # linkIndex=-1 代表查询 base link
        dynamics_info = p.getDynamicsInfo(self.obj_id, -1)
        # dynamics_info[3] 就是 localInertialPosition
        inertial_offset = dynamics_info[3]
        
        # 缩放会影响局部偏移的实际大小
        self.inertial_offset_local = np.array(inertial_offset) * scaling
        
        print(f"成功包装物体 ID:{self.obj_id}。自动检测到惯性偏移: "
              f"{[f'{x:.4f}' for x in self.inertial_offset_local]}")

    def get_base_pose(self):
        """获取基座原点(base link origin)在世界坐标系中的位姿。"""
        com_pos, com_orn = p.getBasePositionAndOrientation(self.obj_id)
        offset_world = p.rotateVector(com_orn, self.inertial_offset_local)
        base_pos = np.array(com_pos) - np.array(offset_world)
        return base_pos, np.array(com_orn)

    def reset_base_pose(self, base_pos, base_orn):
        """ 重置base到指定的位姿(pybullet中把base视为inertial) """
        offset_world = p.rotateVector(base_orn, self.inertial_offset_local)
        com_pos = np.array(base_pos) + np.array(offset_world)
        p.resetBasePositionAndOrientation(self.obj_id, com_pos, base_orn)