...
class PsmsEnv(PsmEnv):
    ...
    def __init__(self,
        ...
        super(PsmsEnv, self).__init__(render_mode, cid=-1)

    def _env_setup(self):
        super(PsmsEnv, self)._env_setup()

        self.psm_1_matrices.clear()
        self.psm_2_matrices.clear()
        self.psm_1_jaw.clear()
        self.psm_2_jaw.clear()
    ...
    def _set_action(self, action: np.ndarray):
        ...
        *2 for PSM1 [0: 5] and PSM2 [5: 10]
        ...
        for i, psm in enumerate((self.psm1, self.psm2)):
            ...
            # Only record for dVRK experiment
            if i == 0:
                self.psm_1_matrices.append(action_rcm)
                self.psm_1_jaw.append(action[4])
            elif i == 1:
                self.psm_2_matrices.append(action_rcm)
                self.psm_2_jaw.append(action[9])
